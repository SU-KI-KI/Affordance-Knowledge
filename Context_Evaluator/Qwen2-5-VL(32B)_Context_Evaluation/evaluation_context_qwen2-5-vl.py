...
def _balance_json_brackets(text: str) -> str:
    """
    Repair common truncated JSON from LLM output:
    - missing closing ] before }
    - missing closing } at the end
    - missing closing ] at the end
    - trailing comma before } or ]
    - string truncated before closing quote

    This scanner respects JSON strings and repairs mismatched closers,
    e.g. ["a", "b" }  -> ["a", "b" ] }
    """
    s = _remove_trailing_commas((text or "").strip())
    ...
    for ch in s:
        ...
        if ch in "}]":
            ...
            if stack and stack[-1] == expected_open:
                ...
                # 多出来的关闭符，保留它通常会导致 JSON 仍然坏掉，所以跳过
                # 如果你想保守保留，可以改成 out.append(ch)
                continue
...
def _try_load_json_with_repair(raw_candidate: str) -> Tuple[Optional[Any], Optional[str], str]:
    """
    Try strict JSON parsing first. If it fails, try bracket repair.

    Returns:
        loaded_json: parsed object or None
        message: None on strict success, repair info on repaired success, error on failure
        used_text: original or repaired JSON text
    """
    try:
        return json.loads(raw_candidate), None, raw_candidate
    except Exception as first_error:
        repaired = _balance_json_brackets(raw_candidate)
        try:
            return json.loads(repaired), f"JSON repaired after parse failure: {first_error}", repaired
        except Exception as second_error:
            return (
                None,
                f"Failed to parse model output as JSON: {first_error}; repair also failed: {second_error}",
                repaired,
            )


def _normalize_confidence_value(confidence: Any) -> Any:
...
def format_evaluator_value(value: Any) -> Any:
    ...
    if isinstance(value, list):
        ...
        if len(formatted) >= 2:
            formatted[1] = _normalize_confidence_value(formatted[1])
        return formatted
    if value is None:
        return ""
    return value
...
def _validate_evaluator_output(parsed: Dict[str, Any]) -> List[str]:
    ...
    for key in EXPECTED_KEYS:
        ...
        if not isinstance(normalized_confidence, float) or not 0.0 <= normalized_confidence <= 1.0:
...
def parse_vlm_json_output(text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    loaded, repair_message, _used_json_text = _try_load_json_with_repair(raw_candidate)

    if loaded is None:
        parse_errors.append(repair_message or "Failed to parse model output as JSON.")
    elif isinstance(loaded, dict):
        parsed = loaded
        parse_errors.extend(_validate_evaluator_output(parsed))

        # By default, successful automatic repair is treated as OK, because the
        # normalized Evaluator_Output is valid and can be safely saved.
        # If you want repaired outputs to appear in error_csv, uncomment below:
        # if repair_message:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate revised affordance context/persona relevance and impact with Qwen2.5-VL.")
    ...
    parser.add_argument("--output_json", type=str, default="Output/output_context_Qwen2-5-VL(32B)_GPT4o_verified.json")
