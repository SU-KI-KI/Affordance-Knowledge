...
DEFAULT_MODEL = os.environ.get("QWEN2_5_VL_MODEL", "Qwen/Qwen2.5-VL-7B-Instruct")
...
def normalize_ra_negligible_to_not_relevant(evaluator_output: Dict[str, Any]) -> Dict[str, Any]:
    """
    Fix a common label-type mismatch from the VLM.

    RA-* fields are relevance-assessment fields, so their valid labels are:
    Highly relevant / Relevant / Not relevant.

    If the model accidentally uses the IA-only label "Negligible" for an RA-*
    field, convert only that label to "Not relevant" while preserving the
    confidence and explanation.
    """
    for key, value in evaluator_output.items():
        if not key.startswith("RA-"):
            continue
        if isinstance(value, list) and len(value) >= 1 and value[0] == "Negligible":
...
def parse_vlm_json_output(text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    normalized = normalize_ra_negligible_to_not_relevant(normalized)
...
def main():
    ...
    parser.add_argument("--output_json", type=str, default="Output/output_context_Qwen2-5-VL(7B)_GPT4o_verified.json")
