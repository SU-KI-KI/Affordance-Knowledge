...
def _extract_json_candidate(text: str) -> str:
    """Extract the JSON-looking part from VLM output.

    VLMs sometimes add prose before/after JSON, or stop before the final
    closing brace.  For truncated outputs we still return the substring from
    the first opening brace so the repair step can balance missing brackets.
    """
...
def _remove_trailing_commas(text: str) -> str:
    """Remove JSON-invalid trailing commas before ] or }."""
    return re.sub(r",\s*([\]}])", r"\1", text)




def _close_arrays_before_next_key(text: str) -> str:
    """Insert a missing ] when the next evaluator key starts inside an array.

    Example VLM error:
        "RA-3": ["Relevant", "0.70", "...", "IA-3": [...]
    becomes:
        "RA-3": ["Relevant", "0.70", "..."], "IA-3": [...]
    """
    key_pattern = r'"(?:RA|IA)-[1-6]"\s*:'
    return re.sub(r'(?<!\])\s*,\s*(' + key_pattern + r')', r'], \1', text)

def _balance_json_brackets(text: str) -> str:
    """Repair common VLM truncation errors such as a missing ] before }.

    The function is conservative: it only inserts missing closing brackets /
    braces needed to make the bracket stack valid, while respecting quoted
    strings and escaped characters. This fixes outputs like:

        "IA-6": ["Marginal", "0.70", "..."
        }

    by converting the final `}` to `]}`.
    """
    opener_to_closer = {"{": "}", "[": "]"}
    closer_to_opener = {"}": "{", "]": "["}
    stack: List[str] = []
    ...
    for ch in text:
        ...
        if ch in closer_to_opener:
            ...
            inserted = []
            while stack and stack[-1] != expected_opener:
                inserted.append(opener_to_closer[stack.pop()])
            if inserted:
                # Insert missing closers immediately before the actual closer.
                out[-1:-1] = inserted
            if stack and stack[-1] == expected_opener:
                stack.pop()

    while stack:
        out.append(opener_to_closer[stack.pop()])

    return "".join(out)


def _json_loads_with_repair(text: str) -> Tuple[Optional[Any], Optional[str]]:
    """Try strict json.loads first, then common VLM-output repairs."""
    candidates: List[Tuple[str, str]] = []
    candidates.append(("strict", text))
    candidates.append(("remove_trailing_commas", _remove_trailing_commas(text)))
    candidates.append(("close_arrays_before_next_key", _close_arrays_before_next_key(text)))
    candidates.append(("balance_brackets", _balance_json_brackets(text)))
    candidates.append(("close_arrays_before_next_key+balance_brackets", _balance_json_brackets(_close_arrays_before_next_key(text))))
    candidates.append(("remove_trailing_commas+balance_brackets", _balance_json_brackets(_remove_trailing_commas(text))))
    candidates.append(("remove_trailing_commas+close_arrays_before_next_key+balance_brackets", _balance_json_brackets(_close_arrays_before_next_key(_remove_trailing_commas(text)))))
    ...
    for method, candidate in candidates:
        ...
        try:
            ...
            return loaded, None if method == "strict" else f"Repaired malformed JSON using {method}."
        except Exception as e:
            last_error = e

    return None, f"Failed to parse model output as JSON after repair: {last_error}"
...
def parse_vlm_json_output(text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    loaded, repair_message = _json_loads_with_repair(raw_candidate)
    if repair_message:
        parse_errors.append(repair_message)

    if isinstance(loaded, dict):
        parsed = loaded
        parse_errors.extend(_validate_evaluator_output(parsed))
    elif loaded is None:
        parse_errors.append("Model output could not be parsed as a JSON object.")
    else:
    ...
    # Bracket repair is expected for vLLM truncation and should not mark the
    # sample as failed when the repaired object passes schema validation.
    non_repair_errors = [err for err in parse_errors if not err.startswith("Repaired malformed JSON")]
    parse_error = "; ".join(non_repair_errors) if non_repair_errors else None
...
def main():
    ...
    parser.add_argument("--output_json", type=str, default="Output/output_context_Qwen3-VL(32B)_GPT4o_verified.json")
