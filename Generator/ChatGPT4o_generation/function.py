...
def generate_image_affordance(prompt, object_names, image_path):
    try:
        ...
        response = client.chat.completions.create(
            model="chatgpt-4o-latest",  # Model selection; make sure your account has access
            messages=[
                {"role": "system", "content": [{"type": "text", "text": prompt}]},
                {"role": "user", "content": [{"type": "text", "text": object_names},
                                             {"type": "image_url", "image_url": {"url": f"{image_path}"}}]}
            ],
            # system: The system message serves as the model's top-level instruction,
            # usually describing what the model should do and how it should behave and respond.
            # user: The user message contains instructions requesting a specific type of output from the model.
        )
        print(response)
