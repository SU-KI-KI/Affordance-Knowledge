...
6. Affordance-centric inference: Affordance-centric inference refers to a series of inferences that are associated with specific affordance knowledge. The key dimensions of affordance-centric inference are:
...
(2) Action duration: Action duration refers to the length of time that an action takes to be performed, encompassing the period from its initiation to its completion. We define five types of action duration, namely seconds, minutes, hours, days, and longer.
...
# Constraints
1. Please generate accurate outputs. For each image, generate 1–5 affordance entries.
