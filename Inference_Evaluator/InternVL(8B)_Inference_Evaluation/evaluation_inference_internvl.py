...
def _looks_like_keyed_evaluator_body(text: str) -> bool:
    """Return True when text contains AA/PA JSON-object-style key/value pairs."""
    return bool(re.search(r'"(?:AA|PA)-[1-5]"\s*:', text or ""))


def _repair_top_level_array_object_mix(text: str) -> str:
    """
    Fix a common VLM JSON error:
    ...
    The above is invalid because it uses object key/value pairs inside a
    top-level array. Convert only this specific pattern to a JSON object.
    """
    s = _strip_code_fence(text).strip()
    if not s:
        return s

    # If there is already a complete JSON object candidate, keep the existing behavior.
    start_obj = s.find("{")
    end_obj = s.rfind("}")
    if start_obj != -1 and end_obj != -1 and end_obj > start_obj:
        ...
        if start_arr != -1 and end_arr != -1 and end_arr > start_arr:
            ...
            if _looks_like_keyed_evaluator_body(candidate):
                candidate = "{" + candidate[1:-1].strip() + "}"
            s = candidate

    # Remove a trailing comma before a closing brace/bracket, another common VLM mistake.
    s = re.sub(r",\s*([}\]])", r"\1", s)
    return s
...
def parse_vlm_json_output(text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    try:
        ...
        repaired_candidate = _repair_top_level_array_object_mix(text)
        try:
            loaded = json.loads(repaired_candidate)
            if isinstance(loaded, dict):
                parsed = loaded
                parse_error = None
            else:
                parse_error = "Model output is not a JSON object after repair."
        except Exception as second_error:
            parse_error = (
                f"Failed to parse model output as JSON: {first_error}; "
                f"repair also failed: {second_error}"
            )
...
def main():
    ...
    parser.add_argument("--output_json", type=str, default="Output/output_inference_InternVL3-5(8B)_GPT4o_verified.json")
